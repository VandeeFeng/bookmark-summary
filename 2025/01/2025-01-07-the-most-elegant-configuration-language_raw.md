...
Markdown Content:
> _“If nothing magically works, nothing magically breaks”_ © [Carson Gross](https://x.com/htmx_org/status/1491418565526061058)
...
What I _don’t want_ is reading a 1000-page book explaining all the edge cases and undefined behaviours happening in the process.
...
The config is just some data written in a file. So the simplest config would be just a string. But just a string is not useful. It doesn’t have _enough_ structure. It’s too flexible.
...
List as key-valuesUnder the hood, it’s just a list of key-value pairs but nobody said keys or values have to be non-empty or even unique.
...
Sections as key-valuesThe above example may look a bit overwhelming but in fact, it’s again just a list of key-value pairs.
...
Cards on the tableIt’s time to come clear finally. The above example raises an unsettling question:
...
But second, and most important, if `parse` is a true _monoid homomorphism_ then it doesn’t matter if we concat files first and then parse or if we first parse and then concat. The result will be the same!
...
Pedantic Alert #1: Error-handlingTo simplify the explanation, I made an assumption that the function `parse` doesn’t fail. In practice, it’s absolutely possible for parsing to fail on invalid inputs. Does it mean we can’t benefit from monoid homomorphisms here? Of course not!
...
The quick answer: DIY. When you parse the final config (or parse first and then append lists, remember _monoid homomorphisms_), you’ll get the following list of key-value pairs:
...
Currently, I’m focusing on building [GitHub TUI](https://github.com/chshersh/github-tui) in OCaml. If at some point the need for a config arises, I return to `ccl`.
...
> *   [𝕏: @chshersh](https://x.com/ChShersh)
